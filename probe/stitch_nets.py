...
def _parse_args(args):
    ...
    parser.add_argument('front_model', nargs='?', help='Path first model', type=str)
    parser.add_argument('end_model', nargs='?', help='Path to second model', type=str)
    parser.add_argument('front_layer', nargs='?', help='Last layer of first model', type=str)
    parser.add_argument('end_layer', nargs='?', help='Last layer of second model', type=str)
    ...
    parser.add_argument('--num_tasks', help='Number of tasks', type=int, default=1)
    parser.add_argument('--task_idx', help='Index of task', type=int, default=0)

    parser.add_argument('--gin_file', help='Gin config file', type=str, default=None)
...
def create_data(conf, trainer, trans_m_before_train):
    ...
    m1_loss_on_task, m1_acc_on_task, m1_hits_on_task = eval_net(m1_runner, conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks))
    m2_loss_on_task, m2_acc_on_task, m2_hits_on_task = eval_net(m2_runner, conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks))
    trans_loss_on_task, trans_acc_on_task, frank_hits_on_task = eval_net(trainer.model_trainer, conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks))
    m1_m2_rel_results_on_task = relative_eval_net(frank_model.front_model,
                                          frank_model.end_model,
                                          out_data['params']['dataset']+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks),
                                          verbose=False)
    ...
    out_data['model_results'] = {
        ...
        'front_on_task': {
            'loss': m1_loss_on_task,
            'acc': m1_acc_on_task
        },
        'end_on_task': {
            'loss': m2_loss_on_task,
            'acc': m2_acc_on_task
        },
        'trans_on_task': {
            'loss': trans_loss_on_task,
            'acc': trans_acc_on_task
        }
    }
    ...
    if conf.low_rank is None:
        ...
        comparator_on_task = ActivationComparator.from_frank_model(frank_model)
        comparator_frank = ActivationComparator.from_frank_model(frank_model, 'frank', 'end')
        comparator_frank_on_task = ActivationComparator.from_frank_model(frank_model, 'frank', 'end')
        ...
        frank_sim_on_task = frank_m2_similarity(frank_model, out_data['params']['dataset']+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks), verbose=False)

        print('\n','='*80)
        print('M1 --> M2\n')
        measures = comparator(conf.dataset, ['cka', 'ps_inv', 'l2'], batch_size, group_at, stop_at, dataset_type)
        measures_on_task = comparator_on_task(conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks), ['cka', 'ps_inv', 'l2'], batch_size, group_at, stop_at, dataset_type)
        ...
        measures_frank_on_task = comparator_frank_on_task(conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks), ['cka', 'l2'], batch_size, group_at, stop_at, dataset_type)
        ...
        out_data['cka_on_task'] = measures_on_task['cka']
        out_data['cka_frank_on_task'] = measures_frank_on_task['cka']

        # l2
        out_data['l2'] = measures['l2']
        out_data['l2_frank'] = measures_frank['l2']
        out_data['l2_on_task'] = measures_on_task['l2']
        out_data['l2_frank_on_task'] = measures_frank_on_task['l2']
        ...
        w_on_task = measures_on_task['ps_inv']['w'][..., None, None]
        b_on_task = measures_on_task['ps_inv']['b']
        out_data['trans_m']['ps_inv_on_task'] = {'w': w_on_task, 'b': b_on_task}

        diff_on_task = out_data['trans_m']['ps_inv_on_task']['w'] - out_data['trans_m']['after']['w']
        out_data['trans_m']['ps_frank_on_task'] = {'w': diff_on_task}
        ...
        # psinv loss
        ps_inv_model_trainer_on_task = FrankStepHandler.from_data_dict(out_data, 'ps_inv')
        ps_inv_loss_on_task, ps_inv_acc_on_task, ps_inv_hits_on_task = eval_net(ps_inv_model_trainer_on_task, conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks))

        ps_inv_model_on_task = ps_inv_model_trainer_on_task.model
        comparator_ps_inv_on_task = ActivationComparator.from_frank_model(ps_inv_model_on_task, 'frank', 'end')
        ps_inv_sim_on_task = frank_m2_similarity(ps_inv_model_on_task, out_data['params']['dataset'], verbose=False)

        print('='*80)
        print('M1 + Least Squares Stitch --> M2\n')
        measures_ps_inv_on_task = comparator_ps_inv_on_task(conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks), ['cka', 'l2'], batch_size, group_at, stop_at, dataset_type)
        print_measures = measures_ps_inv_on_task.copy()
        print_measures.update(ps_inv_sim_on_task)
        print('\n', {k:round(v,3) for (k,v) in print_measures.items() if k!="same_class_out"})

        # Cka
        out_data['cka_ps_inv_on_task'] = measures_ps_inv_on_task['cka']

        # l2
        out_data['l2_ps_inv_on_task'] = measures_ps_inv_on_task['l2']

        out_data['model_results']['ps_inv_on_task'] = {'loss': ps_inv_loss_on_task, 'acc': ps_inv_acc_on_task}
        ...
        out_data['hits']['m2_frank_on_task'] = get_hits_overlap(m2_hits_on_task, frank_hits_on_task)
        out_data['hits']['m2_ps_inv_on_task'] = get_hits_overlap(m2_hits_on_task, ps_inv_hits_on_task)
        out_data['hits']['frank_ps_inv_on_task'] = get_hits_overlap(frank_hits_on_task, ps_inv_hits_on_task)


        # Similarities
        out_data['m2_sim'] = {'ps_inv': ps_inv_sim, 'after': frank_sim, 'm1': m1_m2_rel_results}
        out_data['m2_sim_on_task'] = {'ps_inv': ps_inv_sim_on_task, 'after': frank_sim_on_task, 'm1': m1_m2_rel_results_on_task}
...
def run(conf):
    ...
    # Define dataset
    datasets = get_datasets(conf.dataset+"_TASK_{}_OF_{}".format(conf.task_idx+1, conf.num_tasks))
...
def main(args=None, dataset=gin.REQUIRED, init=gin.REQUIRED, front_model=gin.REQUIRED, end_model=gin.REQUIRED, front_model_task_idx=gin.REQUIRED, end_model_task_idx=None, layer=gin.REQUIRED, model_num_epochs=gin.REQUIRED, task_idx=gin.REQUIRED, num_tasks=gin.REQUIRED):

    args.dataset = dataset
    args.init = init

    args.front_model = front_model.replace('${front_model_task_id}', str(front_model_task_idx+1)).replace('${model_num_epochs}', str(model_num_epochs))
    args.end_model = end_model.replace('${end_model_task_id}', str(end_model_task_idx+1)).replace('${model_num_epochs}', str(model_num_epochs))

    args.front_layer = layer
    args.end_layer = layer
