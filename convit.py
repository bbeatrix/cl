...
@gin.configurable(denylist=['input_shape', 'output_shape'])
class ConVit(nn.Module):
    """ Vision Transformer with support for patch or hybrid CNN input stage
    """
    def __init__(self, input_shape, output_shape, emb_dim, use_classifier_head, pretrained, freeze_base, freeze_top,
        ...
        super().__init__()
        print(f'ConViT with {locals()}')
        print("params:", patch_size, in_chans, num_classes, embed_dim, depth, num_heads, mlp_ratio, qkv_bias, qk_scale, drop_rate, attn_drop_rate, drop_path_rate, hybrid_backbone, norm_layer, local_up_to_layer, locality_strength, use_pos_embed, class_attention, ca_type)
        in_chans = input_shape[0]
        img_size = input_shape[-1]
        num_classes = output_shape[0]
        assert len(output_shape) == 1, 'Only one output head is supported in convit now'
    ...
    def features(self, x, final_norm=True):
        B = x.shape[0]
        x = self.patch_embed(x)
