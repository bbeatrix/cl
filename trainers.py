...
def trainer_maker(target_type, *args):
    ...
    elif target_type == "supervised with ewc":
        return EWCTrainer(*args)
...
class Trainer:
    ...
    def _log_avg_accuracy_and_forgetting(self):
        with torch.no_grad():
            ...
            np.savetxt(
                save_path, np.array([np.array(v) for v in self.task_accuracies.values()]), delimiter=", ", fmt="%s"
            )
        return
...
class SupTrainerWForgetStats(SupTrainer):
    ...
    def update_forget_stats_and_scores(self, idxs, corrects, softmax_preds, pred_scores):
        ...
        softforget2_conditions = (self.forget_stats["prev_softmaxpreds"][idxs] > self.softforget_pred_threshold2) & (
            softmax_preds < self.softforget_pred_threshold2
        )
